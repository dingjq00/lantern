...
**P1 扩展**：P1 将 Step 5-7（路由→执行→观察）升级为 ReAct 循环。首轮和 P0 一样批量规划执行；如果结果不够，LLM 基于观察结果补充追查调用（最多 3 轮追查）。9 步骨架不变，只是 Step 5-7 可以重复。详见 Section 5.7。
...
### 5.7 ReAct Agent（P1 实现，已调优）

三步 think 架构 + 5 步结构化模板 + 参数引用 + 级联策略：

```
用户查询
  → [think① 规划] 5 步模板：[意图][候选][审查][规划][验证]
    支持 {{N.path}} 参数引用，多层依赖一轮规划
    级联：mini 失败 → 自动升级 gpt-5.4
  → [Router 执行] 按序执行 calls，自动解析 {{}} 引用传参
  → [think② 审查放开] 结果够 → finish / 不够 → 放开范围补充
  → [Router 执行] 补充调用
  → [think③ finish]
  → [Summarize] 禁止编造数据
```
...
关键特性：
- **5 步结构化思考**：ToolGT + Step-Back + Reflexion 三技术融合
- **参数引用 `{{N.path}}`**：多层依赖一轮完成，router 自动传参
- **三步 think**：简单题 2 次，复杂题 3 次，固定上限
- **级联策略**：mini unsupported/空 calls → 自动升级 5.4
- **域覆盖检查**：[验证] 确认每个识别的域都有工具覆盖
- **summarize 禁止编造**：数据不足时说"数据不足"，不编数字

通用架构（零查找表）3 次平均 Recall 87.5%（GT 修正后单次峰值 91.7%），L1=100% L2=95% L3=~80% L4=~82% L5=~85%。

### 5.8 自学习系统（待重构）

已验证无效的方案：
- verdict（记录 winner 工具链）— 60% 准确率下积累 40% 错误经验
- AI 自评 lesson — 循环论证，同一个脑子犯的错自评也发现不了

待实现的有效方案：
- 用户反馈（thumbs down）→ 记录 query + tools → 写 lesson
- 结果质量检测 → 返回数据和问题不匹配 → 自动标记
- Ground Truth 对比（测试阶段）→ 自动注入正确路径
- Skill 分离（意图解析/工具匹配/结果校验 独立可调）
...
| Agent 循环 | 单次路由 | P1 启用 ReAct Agent（观察→决定→追查） |
