...
### 5.7 ReAct Agent（P1 实现）

P0 是一次规划模式（规划→批量执行→总结）。P1 升级为 ReAct 模式（批量调用 + 按需追查）：

```
用户查询 → [规划] 一次规划完整工具链（可多个，和 P0 兼容）
         → [批量执行] 按序执行所有工具调用
         → [观察] 结果够不够？
                  够 → 直接总结（和 P0 一样快）
                  不够 → [追查] 基于已有结果补充新的工具调用
         → [批量执行] 执行补充调用
         → ... 最多 3 轮追查（共 4 轮）
         → [总结] 汇总所有结果
```

LLM 每轮输出结构化 ThinkResult（thought + calls[] + finish/unsupported），不使用 `__continue__` 哨兵。统一流程，无需预判复杂度——复杂度是循环的输出而非输入。

详见 P1 设计文档：`docs/superpowers/specs/2026-03-26-p1-design.md` Section 5。
