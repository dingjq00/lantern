...
export function registerFaultSearch(server: McpServer) {
  server.tool(
    ...
    async (args) => {
      ...
      // 防御: filter 已指定具体值时，groupBy 同维度是多余的（过滤后再分组=100%同值）
      const effectiveGroupBy = (args.groupBy === 'status' && args.status !== undefined)
        || (args.groupBy === 'urgency' && args.urgency !== undefined)
      ...
      if (result.groups) {
        // MTBF 时间间隔预计算 — 在 enrichGroupNames 之前执行
        if (args.groupBy === 'equipment') {
          enrichGroupsWithIntervals(list, result.groups, 'equipmentId', 'reportTime')
        }
